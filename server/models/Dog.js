...
/* While Mongo is a schema-less database, meaning we can just store arbitrary objects in it, Mongoose
   ...
   variable should contain. We also give it the required option, which means every object in the Dog database MUST
   have a name. We set the trim option to true, which will remove trailing and leading whitespace from the string.
   Finally we set the unique option to true. This means that there cannot be two Dogs with the same name.

   For our bedsOwned, we say it is a number. We set the minimum value to 0. We say it is required.

   For our createdDate, we want to store the time that the Dog was first added to the database. Rather than
   figure that out in our own code when we create each Dog, we can instead set it's default to the Date.now function.
   When a new Dog is made, it will execute the default function and return the current date.
*/
